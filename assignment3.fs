...
(* Student name: Shah Tausif Iqbal, Id Number: 260515282 *) (* Edit this line. *)
...
let rec insert comp (item: int) (list: RList) =
    let destructiveCell = {data = item; next = ref None}
    match !list with
    | None -> list := Some destructiveCell
    | Some {data = d; next = nextCells} ->
        if (comp(d, item)) then
            destructiveCell.next := !nextCells
            nextCells := Some destructiveCell
        else insert comp item nextCells
...
let make_protected_account (opening_balance : int , password: string) =
    ...
    fun (s:string, t:transaction) ->
      match s, t with
        |g, Withdraw(m) -> if (password.Equals(g)) then
                              if(!balance > m) then
                                balance := !balance - m
        ...
        |g, Deposit(m) -> if(password.Equals(g)) then
                            balance := !balance + m
                            printfn "The new balance is %i" !balance
                          else
